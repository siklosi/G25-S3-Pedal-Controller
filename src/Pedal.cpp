...
void Pedal::update() {
    ...
    // For extreme smoothness, we can take multiple samples here immediately
    long sum = 0;
    int samples = 16;
    ...
    int effMin = _config.min + _config.deadzoneStart;
    int effMax = _config.max - _config.deadzoneEnd;
    
    // Protect against division by zero or weird config
    if (effMax <= effMin) effMax = effMin + 1;

    int constrainedRaw = constrain(rawAvg, effMin, effMax);
    
    // 3. Map to 0-1000 (0.0% to 100.0%) for Curve lookup
    // Using 0-1000 gives us 0.1% resolution on the input side for the curve
    long inputScaled = map(constrainedRaw, effMin, effMax, 0, 1000);
    if (_config.inverted) {
    ...
    int remainder = inputScaled % 100; // 0..99
    ...
    // "Limiter" usually means clip, but for racing, re-scaling range is often better so you still have full pedal travel.
    // However, brake limit usually means "I press 100% physically but game sees 80%". 
    // Wait, usually it's "I want 100% game signal at 80% pressure". That's what calibration Min/Max is for.
    // "Output Limiter" as requested (cap max output) usually means "Don't ever send more than 80% to game".
    // This is useful if a car locks up easily. So we Clip or Rescale.
    // Let's CLIP. If result > ceiling, result = ceiling.
    if (interpolated > _config.outputCeiling) interpolated = (float)_config.outputCeiling;
    ...
    // alpha = 1.0 means no filtering (take new value). alpha = 0.01 means mostly old value.
    // Map smoothing 0 (alpha 1.0) -> 90 (alpha 0.1) -> 99 (alpha 0.01)
...
void Pedal::calibrateMin() {
    _config.min = _currentRaw;
}

void Pedal::calibrateMax() {
    _config.max = _currentRaw;
}
